...
3.  Apart from the US, most countries use fuel consumption (fuel consumed over fixed distance) rather than fuel economy (distance travelled with  fuel). How could you convert `cty` and `hwy` into the European standard of l/100km? 
...
In the first plot, the value "blue" is scaled to a pinkish color, and a legend is added. In the second plot, the points are given the R color blue. 
...
1.  How is drive train related to fuel economy? How is drive train related to engine size and class?
...
5.  You now know (at least) three ways to compare the distributions of subgroups: `geom_violin()`, `geom_freqpoly()` and the color aesthetic, or `geom_histogram()` and facetting. What are the strengths and weaknesses of each approach? What other approaches could you try?

6.  Read the documentation for `geom_bar()`. What does the `weight` aesthetic do? 

7.  Using the techniques already discussed in this workshop, come up with three ways to visualise a 2d categorical distribution. Try them out by visualising the distribution of `model` and `manufacturer`, `trans` and `class`, and `cyl` and `trans`.
...
To further your learning be sure to check out the following free(!) online resources:

- [The R Graph Gallery](https://www.r-graph-gallery.com/): Extremely useful repository of data visualizations and the R code necessary to implement them.
